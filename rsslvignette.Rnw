...
<<generateddatasets, echo=FALSE, warning=FALSE, fig.cap="Simulated Datasets. Each can be generated using a function of the form \\texttt{generate*}, where \\texttt{*} should be replaced by the name of the dataset. (alt) indicates non-default parameters where used when calling the function.",fig.height=4, fig.width=4>>=
library(RSSL)
...
<<selflearning, echo=TRUE, cache=TRUE,warning=FALSE, size='footnotesize'>>=
library(RSSL)
set.seed(1)
...
<<labelpropagation, dev='pdf', echo=FALSE, fig.cap="Replication of Figure 2 from \\cite{Zhu2003} demonstrating harmonic energy minimization. The larger points indicate the labeled objects. The color indicates the predicted class.",fig.subcap=c('Parallel planes dataset', 'Spirals dataset'), out.width='.48\\linewidth',fig.height=6.5>>=
library(RSSL)
library(dplyr)
library(ggplot2)
...
<<lowdensityproblem, echo=FALSE, fig.cap="Demonstration of potential problems when the low density assumption does not hold, similar to Figure 6.5 in \\cite{Zhu2009}", fig.subcap=c("Low-density assumption","Non low-density assumption"), out.width='.49\\linewidth', fig.width=5, fig.height=5,size='footnotesize'>>=
library(RSSL)
library(ggplot2)
...
<<improvementguarantees, echo=TRUE,results='hold',size='footnotesize',tidy=FALSE>>=
library(RSSL)
set.seed(1)
